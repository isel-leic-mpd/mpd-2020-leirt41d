...
import static utils.StreamUtilsAula13.*;



public class StreamTestsAula13 {
    ...
    @Test
    public void collapse2AnotherTest() {
        Stream<Integer> collapsed =
        collapse1(
                IntStream.of(2, 2, 1, 3, 4, 4, 4, 1, 1, 2, 2, 2)
                        .boxed());
        ...
        Spliterator<Integer> it = collapsed.spliterator();


        while(it.tryAdvance(__ -> {})) count++;

        assertEquals(expectedCount, count);
    }

    @Test(timeout=10000)
    public void collapse3AnotherTest() {
        Random r = new Random();
        Stream<Integer> random =
                Stream.iterate(1, i ->  i +1);
        Stream<Integer> collapsed =
                collapse1(random)
                        .limit(10);
